# Hi, I'm Isaiah Herard 👋🏽

## 🎯 IAM Operations Engineer | CyberArk PAM Specialist | Linux Administrator

I'm an experienced security professional specializing in **Azure Cloud Security**, **CyberArk Privileged Access Management (PAM)**, and **Red Hat Enterprise Linux**. This GitHub showcases my hands-on labs, projects, and technical expertise.

---

## 🔐 My Specializations

```
┌─────────────────────────────────────────────────────────────────────────┐
│                        ENTERPRISE SECURITY STACK                         │
├─────────────────────┬─────────────────────┬─────────────────────────────┤
│   ☁️ CLOUD SECURITY  │   🔑 PAM SECURITY    │   🐧 LINUX SECURITY         │
│                     │                     │                             │
│   Microsoft Azure   │   CyberArk PAS      │   Red Hat Enterprise Linux  │
│   • Entra ID        │   • Vault           │   • RHEL 8/9                │
│   • Defender        │   • PSM             │   • SELinux                 │
│   • Sentinel        │   • CPM             │   • Hardening               │
│   • Conditional     │   • PVWA            │   • Ansible                 │
│     Access          │   • CCP/AAM         │   • Identity Management     │
└─────────────────────┴─────────────────────┴─────────────────────────────┘
```

---

## 🛠️ Skills & Tools

### ☁️ Cloud Platforms
![Azure](https://img.shields.io/badge/Microsoft_Azure-0078D4?style=for-the-badge&logo=microsoft-azure&logoColor=white)
![Entra ID](https://img.shields.io/badge/Microsoft_Entra_ID-0078D4?style=for-the-badge&logo=microsoft&logoColor=white)
![Microsoft 365](https://img.shields.io/badge/Microsoft_365-D83B01?style=for-the-badge&logo=microsoft-office&logoColor=white)
![Active Directory](https://img.shields.io/badge/Active_Directory-003366?style=for-the-badge&logo=windows&logoColor=white)

### 🔑 Privileged Access Management
![CyberArk](https://img.shields.io/badge/CyberArk-6C3C9D?style=for-the-badge&logoColor=white)
![CyberArk Vault](https://img.shields.io/badge/CyberArk_Vault-6C3C9D?style=for-the-badge&logoColor=white)
![CyberArk PSM](https://img.shields.io/badge/CyberArk_PSM-6C3C9D?style=for-the-badge&logoColor=white)
![CyberArk CPM](https://img.shields.io/badge/CyberArk_CPM-6C3C9D?style=for-the-badge&logoColor=white)
![Secrets Management](https://img.shields.io/badge/Secrets_Management-green?style=for-the-badge)

### 🐧 Linux & Automation
![Red Hat](https://img.shields.io/badge/Red_Hat_Enterprise_Linux-EE0000?style=for-the-badge&logo=redhat&logoColor=white)
![RHEL](https://img.shields.io/badge/RHEL_8/9/10-EE0000?style=for-the-badge&logo=redhat&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=for-the-badge&logo=ansible&logoColor=white)
![Bash](https://img.shields.io/badge/Bash_Scripting-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white)
![SELinux](https://img.shields.io/badge/SELinux-EE0000?style=for-the-badge&logoColor=white)

### 🌐 Networking
![Cisco](https://img.shields.io/badge/Cisco-1BA0D7?style=for-the-badge&logo=cisco&logoColor=white)
![Networking](https://img.shields.io/badge/TCP/IP_Networking-005571?style=for-the-badge)
![Firewall](https://img.shields.io/badge/Firewalls-FF6B00?style=for-the-badge)

### 🛡️ Security Operations
![SIEM](https://img.shields.io/badge/SIEM/SOAR-orange?style=for-the-badge)
![Microsoft Sentinel](https://img.shields.io/badge/Microsoft_Sentinel-0078D4?style=for-the-badge&logo=microsoft&logoColor=white)
![Defender](https://img.shields.io/badge/Microsoft_Defender-00A4EF?style=for-the-badge&logo=microsoft&logoColor=white)
![Incident Response](https://img.shields.io/badge/Incident_Response-DC143C?style=for-the-badge)
![Threat Hunting](https://img.shields.io/badge/Threat_Hunting-8B0000?style=for-the-badge)

### 🔒 Security Frameworks & Concepts
![Zero Trust](https://img.shields.io/badge/Zero_Trust-000000?style=for-the-badge)
![IAM](https://img.shields.io/badge/Identity_&_Access_Management-blue?style=for-the-badge)
![NIST](https://img.shields.io/badge/NIST_Framework-003366?style=for-the-badge)
![Risk Management](https://img.shields.io/badge/Risk_Management-4B0082?style=for-the-badge)

### 📋 Project Management
![PMP](https://img.shields.io/badge/Project_Management-2E8B57?style=for-the-badge)
![Agile](https://img.shields.io/badge/Agile/Scrum-0095D5?style=for-the-badge)

---

## 📜 Certifications

### 🛡️ SANS/GIAC
| Certification | Description | Status |
|--------------|-------------|--------|
| ![GIAC](https://img.shields.io/badge/GFACT-004088?style=flat-square) **GFACT** | GIAC Foundational Cybersecurity Technologies | ✅ Certified |
| ![GIAC](https://img.shields.io/badge/GSEC-004088?style=flat-square) **GSEC** | GIAC Security Essentials | ✅ Certified |
| ![GIAC](https://img.shields.io/badge/GCIH-004088?style=flat-square) **GCIH** | GIAC Certified Incident Handler | ✅ Certified |

### ☁️ Microsoft Azure
| Certification | Description | Status |
|--------------|-------------|--------|
| ![Azure](https://img.shields.io/badge/AZ--104-0078D4?style=flat-square) **AZ-104** | Azure Administrator Associate | ✅ Certified |
| ![Azure](https://img.shields.io/badge/SC--300-0078D4?style=flat-square) **SC-300** | Identity and Access Administrator Associate | ✅ Certified |
| ![Azure](https://img.shields.io/badge/AZ--500-0078D4?style=flat-square) **AZ-500** | Azure Security Engineer Associate | ✅ Certified |
  ![M365](https://img.shields.io/badge/MS--102-0078D4?style=flat-square)  **MS-102** | Microsoft 365 Administrator - Expert  | ✅ Certified |

### 🔑 CyberArk
| Certification | Description | Status |
|--------------|-------------|--------|
| ![CyberArk](https://img.shields.io/badge/Defender-6C3C9D?style=flat-square) **CyberArk Defender** | PAM Administration & Configuration | ✅ Certified |
| ![CyberArk](https://img.shields.io/badge/Sentry-6C3C9D?style=flat-square) **CyberArk Sentry** | PAM Engineering & Implementation | ✅ Certified |
| ![CyberArk](https://img.shields.io/badge/Guardian-6C3C9D?style=flat-square) **CyberArk Guardian** | PAM Advanced Security | ✅ Certified |

### 🌐 Networking
| Certification | Description | Status |
|--------------|-------------|--------|
| ![Cisco](https://img.shields.io/badge/CCNA-1BA0D7?style=flat-square) **CCNA** | Cisco Certified Network Associate | ✅ Certified |

### 🐧 Linux
| Certification | Description | Status |
|--------------|-------------|--------|
| ![Red Hat](https://img.shields.io/badge/RHCSA-EE0000?style=flat-square) **RHCSA** | Red Hat Certified System Administrator | ✅ Certified |
| ![Red Hat](https://img.shields.io/badge/RHCE-EE0000?style=flat-square) **RHCE** | Red Hat Certified Engineer | ✅ Certified |

### 🏆 Professional
| Certification | Description | Status |
|--------------|-------------|--------|
| ![ISC2](https://img.shields.io/badge/CISSP-006400?style=flat-square) **CISSP** | Certified Information Systems Security Professional | ✅ Certified |
| ![PMI](https://img.shields.io/badge/PMP-4169E1?style=flat-square) **PMP** | Project Management Professional | ✅ Certified |

---

## 📂 Lab Portfolio

### ☁️ Azure Security (AZ-500)

| Lab | Topics | Link |
|-----|--------|------|
| **Identity Security** | MFA, Conditional Access, Entra ID Protection | [View Lab](https://github.com/YOURUSERNAME/az500-identity-lab) |
| **Network Security** | NSGs, Azure Firewall, Bastion, Private Link | 🔜 Coming Soon |
| **Compute Security** | ACR, AKS, VM Security, Disk Encryption | 🔜 Coming Soon |
| **Data Security** | Key Vault, SQL Security, Storage Encryption | 🔜 Coming Soon |
| **Security Operations** | Microsoft Sentinel, Defender for Cloud | 🔜 Coming Soon |

### 🔑 CyberArk PAM

| Lab | Topics | Link |
|-----|--------|------|
| **PAM Fundamentals** | Vault Architecture, PVWA, Safe Management | 🔜 Coming Soon |
| **Privileged Session Manager** | PSM Setup, Session Recording, Connectors | 🔜 Coming Soon |
| **Central Policy Manager** | CPM Configuration, Password Rotation | 🔜 Coming Soon |
| **Application Identity** | CCP, AAM, Secrets Manager | 🔜 Coming Soon |
| **PAM + Azure Integration** | Azure AD Integration, Cloud Connectors | 🔜 Coming Soon |

### 🐧 Red Hat Enterprise Linux

| Lab | Topics | Link |
|-----|--------|------|
| **RHEL Fundamentals** | Installation, Users, Permissions, Services | 🔜 Coming Soon |
| **Security Hardening** | SELinux, Firewalld, SSH Hardening, Auditd | 🔜 Coming Soon |
| **Identity Management** | IdM/FreeIPA, LDAP, Kerberos | 🔜 Coming Soon |
| **Automation with Ansible** | Playbooks, Roles, Security Automation | 🔜 Coming Soon |
| **RHEL + CyberArk** | PSM-SSH, Unix Connector, Credential Management | 🔜 Coming Soon |

---

## 🎯 Why This Combination?

These technologies work together in enterprise environments:

```
┌─────────────────────────────────────────────────────────────────┐
│                    ENTERPRISE SECURITY FLOW                      │
│                                                                  │
│  👤 User/Admin                                                   │
│       │                                                          │
│       ▼                                                          │
│  ┌─────────────────┐    Authenticate    ┌──────────────────┐    │
│  │   Azure/Entra   │◄──────────────────►│    CyberArk      │    │
│  │   Identity      │    MFA + SSO       │    Vault         │    │
│  └─────────────────┘                    └──────────────────┘    │
│       │                                        │                 │
│       │ Cloud Resources                        │ Privileged      │
│       ▼                                        ▼ Credentials     │
│  ┌─────────────────┐                    ┌──────────────────┐    │
│  │  Azure VMs &    │◄───────────────────│   PSM Session    │    │
│  │  Services       │    Secure Access   │   Recording      │    │
│  └─────────────────┘                    └──────────────────┘    │
│       │                                        │                 │
│       ▼                                        ▼                 │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │              RHEL Servers (Backend Infrastructure)       │    │
│  │         • Database Servers    • Application Servers      │    │
│  │         • Web Servers         • Domain Controllers       │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## 📝 Blog Posts & Write-ups

<!-- Update these as you publish -->
- [Mastering Azure Identity Security: MFA, Conditional Access & Entra ID Protection](YOUR_MEDIUM_LINK)
- More coming soon...

---

## 📈 GitHub Activity

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=ZayIAM&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=ZayIAM&layout=compact&theme=tokyonight)

---

## 🤝 Let's Connect!

I'm open to opportunities in:
- 🔐 **PAM Engineering / Architecture**
- ☁️ **Cloud Security Engineering**
- 🐧 **Linux Systems Security**
- 🔑 **Identity & Access Management (IAM)**
- 🛡️ **Security Operations / Incident Response**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/isaiah-herard/)
[![Medium](https://img.shields.io/badge/Medium-12100E?style=for-the-badge&logo=medium&logoColor=white)](https://medium.com/@ZayIAM)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:isaiahherard26@gmail.com)

---

## 💡 About This Portfolio

Each repository contains:
- 📋 **Detailed Documentation** - Step-by-step lab guides
- 📸 **Screenshots** - Evidence of hands-on work
- 💻 **Scripts & Code** - Automation and configuration files
- 🎯 **Exam Tips** - Key concepts for certification prep
- 🔗 **Resources** - Links to official documentation

---

### ⭐ Found something helpful? Give it a star!

*This portfolio is updated regularly with new labs and projects.*
